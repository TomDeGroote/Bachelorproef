...
\While{$\neg\textit{runOutOfTime}$}
	\ForAll{$\textit{curEq} \gets \textit{currentLevel}$}
		\If{getRunOutOfTime()}
			\State $\textit{runOutOfTime} \gets \textit{true}$
			\State break
		\EndIf
		\ForAll{$\textit{nTRule} \gets \textit{CFG}$.getNonTRules()}
			\ForAll{$\textit{TRule} \gets \textit{CFG}$.getTRules()}
...
%\bibliography{Main.bib} %TODO Fix this stuff
