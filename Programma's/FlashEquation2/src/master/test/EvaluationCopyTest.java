...
public class EvaluationCopyTest {
	...
	public void testEvaluateEquation() {
		...
		HashMap<Double, List<Equation>> allPossibleResults = evaluate
				.evaluateEquation(eq1);
	...
	public void testConcatenateResults() {
		...
		Ks.put(StringMaster.getNameOfGoalK(), 4.0);
		evaluate.examples.add(Ks);
		HashMap<Double, List<Equation>> result = evaluate.concatenateResults(
				part1, new Operand("+", true, true, 0), part2);
	...
	public void testAddPossibelSolutions() {
		...
		Assert.assertEquals("K1+K2", evaluate.getBufferSolutions().get(0)
				.toString());
	}
	...
	public void testEvaluateTerminalEquation() {
		Assert.assertEquals(13.5,
				ObjectEvaluate.evaluateTerminalEquation(eqTerminal2), 0.1);
		Assert.assertEquals(15,
				ObjectEvaluate.evaluateTerminalEquation(eqTerminal1), 0.1);
	}
	...
	public void testCalculateTerm() {
		...
		Assert.assertEquals(d,
				ObjectEvaluate.calculateTerm(eqTerminal2.getListOfSymbols()));
	}

	@Test
	public void testSplitOnEverySplitable() {
		List<List<Symbol>> split = ObjectEvaluate
				.splitOnEverySplitable(eqTerminal1);
	...
	public void testSplitSplitableInThreeParts() {
		...
		List<Equation> split = evaluate.splitSplitableTwoParts(eq1);
		...
		split = evaluate.splitSplitableTwoParts(eq3);
