...
public class ObjectTupleEvaluate extends Evaluate {
	 ...
	 * weer overlopen zoals vorige keer
	 */
	...
	public List<Tuple<Equation, Double>> evaluateEquation(Equation eq,
			int example, boolean nonTerminal) {
		int size = eq.getListOfSymbols().size(); // get the length of this
													// equation
		// size 1 -> E
		if (size == 1) {
		...
		// calculate the value of every nonsplitable part of the equation
			...
			for (Equation splitEq : splitEquations(eq)) {
				temp.add(getValueNonSplitableEquation(splitEq, example,
						nonTerminal));
			}
	 ...
	 * @return The concatenated lists and their values
	 */
	private List<Tuple<Equation, Double>> concatenateAll(
			List<List<Tuple<Equation, Double>>> temp, int e) {
		if (temp.size() == 1) {
			for (Tuple<Equation, Double> possibleSolution : temp.get(0)) {
				if (possibleSolution.y
						.equals(examples.get(e)[examples.get(e).length - 1])) {
					if (e == 0) {
		...
		for (Tuple<Equation, Double> first : temp.get(0)) { // first tuple list
			for (Tuple<Equation, Double> snd : temp.get(1)) { // second tuple
																// list
				result.add(concatenateTuples(first, snd)); // concatenates
															// tuple1 and tuple2
			}
		}
		temp.remove(0);
	 ...
	 *            first tuple
	 * @param snd
	 *            Second tuple, should contain an operand as first symbol of
	 *            equation
	 * @return the two concatenated tuples
	 */
	private Tuple<Equation, Double> concatenateTuples(
			Tuple<Equation, Double> first, Tuple<Equation, Double> snd) {
		...
		Double combinationValue = Grammar.getValue(first.y, (Operand) snd.x
				.getListOfSymbols().get(0), snd.y);
		return new Tuple<Equation, Double>(new Equation(combinationSymbols),
				combinationValue);
	}
	 ...
	 * @return A list of Terminal equations and values
	 */
	public List<Tuple<Equation, Double>> getValueNonSplitableEquation(
			Equation eq, int e, boolean nonTerminal) {
		// trivial solutions
		...
		if (nonTerminal) {
			...
			for (int i = 1; i < symbols.size(); i++) {
				if (symbols.get(i).isNonTerminal()) { // for every E
					...
					for (List<Symbol> sym : newEqs) { // for every already made
														// terminal equation
														// (K0+, K1+, ...)
						for (Terminal K : terminalList) { // add every
															// possibility
															// (K0+K0, K0+K1,
															// K1+K0, ...)
							List<Symbol> newEqContinue = new ArrayList<Symbol>(
									sym);
							newEqContinue.add(K);
					...
					newEqs = nextEqs;
				} else if (symbols.get(i).isTerminal()
						|| symbols.get(i).isOperand()) { // if already K or O
															// just add it to
															// all possiblities
		...
		for (List<Symbol> terminalEq : newEqs) {
			result.add(new Tuple<Equation, Double>(new Equation(terminalEq),
					evaluateTerminalEquation(terminalEq)));
		}
		...
		if (firstWasOperand) {
			...
			for (Tuple<Equation, Double> tuple : result) {
				...
				Double realValue = Grammar.evaluateTrivialValue(starter,
						tuple.y);
				realResult.add(new Tuple<Equation, Double>(Grammar
						.convertEq(realEquation), realValue));
			}
			return realResult;
		}
	...
	/**
	 * Evaluates a equation only containing terminals (for the first example
	 * given)
	 * 
	 * @param terminalEq
	 *            The terminal equation to be evaluated
	 * @return The value of the terminal equation
	 */
	private Double evaluateTerminalEquation(List<Symbol> terminalEq) {
		Double result = Grammar.getValue(
				((Terminal) terminalEq.get(0)).getValue(),
				((Operand) terminalEq.get(1)),
				((Terminal) terminalEq.get(2)).getValue());
		for (int i = 3; i < terminalEq.size(); i = i + 2) {
			result = Grammar.getValue(result, (Operand) terminalEq.get(i),
					((Terminal) terminalEq.get(i + 1)).getValue());
		}
		return result;
	}

	/**
	 * Splits an equation in the smallest possible splitable parts e.g.
	 * E*E+E-E/E => {E*E, +E, -E/E}
	 * 
	 * @param eq
	 *            The equation to be split
	 * @return A list of nonTerminal equations that are the smallest possible
	 */
	public List<Equation> splitEquations(Equation eq) {
		List<Equation> splitEquations = new ArrayList<Equation>();
		List<Symbol> symbols = new ArrayList<Symbol>();
		int i = 0;
		// check if first symbol is an operand (should only be splitable
		// operands
		if (eq.getListOfSymbols().get(0).isOperand()) { // +E*E....
			symbols.add(eq.getListOfSymbols().get(i));
			i = 1;
		}
		// scan over equation until we can split it, then split it and scan
		// further
	...
	/**
	 * Evaluates the trivial (non)Terminal equation OE or OE But because O are
	 * operands some special needs are required Possible solutions will be added
	 * to bufferSolutions
	 * 
	 * @param eq
	 *            The trivial equation
	 * @return A list with terminal equations and corresponding values
	 */
	public List<Tuple<Equation, Double>> evaluateTrivalTwo(Equation eq, int e,
			boolean nonTerminal) {
		if (nonTerminal) {
			List<Tuple<Equation, Double>> result = new ArrayList<ObjectTupleEvaluate.Tuple<Equation, Double>>();
			Double[] example = examples.get(e);
			for (int i = 0; i < example.length - 1; i++) { // -1 because last
															// value is goal
				...
				result.add(new Tuple<Equation, Double>(Grammar
						.convertTrivialEq(possibleEq), value));
			}
			return result;
		} else {
			List<Tuple<Equation, Double>> solution = new ArrayList<ObjectTupleEvaluate.Tuple<Equation, Double>>();
			Double value = Grammar.evaluateTrivial(eq);
			solution.add(new Tuple<Equation, Double>(Grammar
					.convertTrivialEq(eq), value));
			return solution;
		}
	}

	/**
	 * Evaluates the trivial equation (Terminal or nonTerminal) Possible
	 * solutions will be added to bufferSolutions
	 * 
	 * @return A list with terminal equations and corresponding values
	 */
	public List<Tuple<Equation, Double>> evaluateTrivialOne(int e,
			boolean nonTerminal, Equation equation) {
		if (nonTerminal) {
			List<Tuple<Equation, Double>> result = new ArrayList<ObjectTupleEvaluate.Tuple<Equation, Double>>();
			Double[] example = examples.get(e);
			for (int i = 0; i < example.length - 1; i++) { // -1 because last
			...
			list.add(new Tuple<Equation, Double>(equation, ((Terminal) equation
					.getListOfSymbols().get(0)).getValue()));
			return list;
		}
	}
	 ...
	 * @return True if it is a possible solution for all other examples False if
	 *         not
	 *
	 */
