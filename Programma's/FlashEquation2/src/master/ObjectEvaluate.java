...
public class ObjectEvaluate {
	 ...
	 * keer en dan weer overlopen zoals vorige keer
	 */
	...
	/**
	 * evaluate continues to evaluate the tree with every time more and more
	 * examples
	 * 
	 * @param Ks
	 ...
	 * @return all possible solutions of this equation
	 */
	public HashMap<Double, List<Equation>> evaluateEquation(Equation eq) {
		...
		if (eq.getListOfSymbols().size() == 1) {
			...
			for (String K : examples.get(examples.size() - 1).keySet()) {
				if (K.equals(ObjectMaster.getNameOfGoalK())) {
					...
					// and put that list with the value of the equation in the
					// result
					List<Equation> kEquation = new ArrayList<Equation>();
					List<Symbol> terminalK = new ArrayList<Symbol>();
					terminalK.add(new Terminal(K, examples.get(
							examples.size() - 1).get(K)));
					kEquation.add(new Equation(terminalK));
					if (result.containsKey(examples.get(examples.size() - 1)
							.get(K))) {
						result.get(examples.get(examples.size() - 1).get(K))
								.add(new Equation(terminalK));
					} else {
						result.put(examples.get(examples.size() - 1).get(K),
								kEquation);
					}
				}
			}
		...
		if (splitEquations.size() == 1) {
			// size will be 1, can happen when there was no splitable equation
			// e.g. +E*E*E
			splitEquations = splitNonSplitableInThreeParts(eq); // TODO get
																// result don't
																// split
		}
		...
		return concatenateResults(solutionPart1, (Operand) splitEquations
				.get(1).getListOfSymbols().get(0), solutionPart2);
	}

	/**
	 * Concatenates all possible combinations of the solution for a first
	 * equation and a second the operand is found on the beginning of the second
	 * equation
	 * 
	 * @return The resulting combination
	 */
	public HashMap<Double, List<Equation>> concatenateResults(
			HashMap<Double, List<Equation>> solutionPart1, Operand operand,
			HashMap<Double, List<Equation>> solutionPart2) {
		HashMap<Double, List<Equation>> result = new HashMap<Double, List<Equation>>();
		...
		for (Double valueSolution1 : solutionPart1.keySet()) {
			...
			for (Double valueSolution2 : solutionPart2.keySet()) {
				List<Equation> equationsValue2 = solutionPart2
						.get(valueSolution2);
				...
				// the resulting value
				if (operand == null) {
					equationsValue1_2 = concatenateEquationLists(
							equationsValue1, equationsValue2);
					value = Grammar.getValueConcatenate(valueSolution1,
							valueSolution2);
				} else {
					...
					equationsValue1_2 = concatenateEquationLists(
							equationsValue1, equationsValue2);
					value = Grammar.getValue(valueSolution1, operand,
							valueSolution2);
				}
				// if the value is the value we are looking for, add it to
				// possible solutions
	...
	/**
	 * Checks if the value is a possible solution for the first example an if so
	 * if the equations are a solution for every example already given
	 * 
	 * @param value
	 *            The value that was calculated for the first example
	 * @param equationsValue1_2
	 *            The possible equations that create this value for the first
	 *            example
	 */
	public void addPossibelSolutions(Double value,
			List<Equation> equationsValue1_2) {
		// extract the value of the first equation
		if (examples.get(examples.size() - 1)
				.get(ObjectMaster.getNameOfGoalK()).equals(value)) {
			List<Equation> equationsToCheck = new ArrayList<Equation>(
					equationsValue1_2);
				...
				for (Equation equation : equationsToCheck) {
					...
					for (Symbol symbol : equation.getListOfSymbols()) {
						if (symbol.isOperand()) {
							...
							// adds the terminal of this example to the equation
							// (indirectly)
							toEvaluateList.add(new Terminal(symbol.toString(),
									Ks.get(symbol.toString())));
						} else {
							// should never happen
						}
					}
					...
					if (evaluateTerminalEquation(equationToEvaluate) == goal) {
						...
						// is not a possible solution for this equation, do
						// nothing
					}
				}
			...
			// all equations who are still in equationsToCheck are possible
			// solutions given the current examples
	 ...
	 * @return True if the goal is met False if the goal is not met
	 */
	public static double evaluateTerminalEquation(Equation equationToEvaluate) {
		...
		// the values of the terms and operands need to be read in the same
		// order
		...
		for (List<Symbol> term : terms) {
			if (term.size() == 1) { // or operand or T
				if (term.get(0).isOperand()) {
					operands.add((Operand) term.get(0)); // in case of operand,
															// save operand
				} else {
					valueTerms.add(((Terminal) term.get(0)).getValue()); // in
																			// case
																			// of
																			// T,
																			// save
		...
		for (int i = 0; i < operands.size(); i++) {
			result = Grammar.getValue(result, operands.get(i),
					valueTerms.get(i + 1));
		}
		return result;
	}
	 ...
	 * 
	 * @param term
	 *            The term to be evaluated Contains only non splitable operands
	 *            Starts with an Terminal then a Operand then a Terminal and so
	 *            on...
	...
	public static Double calculateTerm(List<Symbol> term) {
		...
		for (int i = 1; i < term.size(); i = i + 2) {
			result = Grammar.getValue(result, (Operand) term.get(i),
					((Terminal) term.get(i + 1)).getValue());
		}
		return result;
	}
	 ...
	 * @return list with all possible combinations of the two list with in
	 *         between the operand
	 *
	 */
	public List<Equation> concatenateEquationLists(
	 ...
	 * 
	 * @return {eq1.getListOfSymbols, operand, eq2.getListOfSymbols}
	 */
	 ...
	 * @return A list containing three equations eq1: first part of equation
	 *         (length will be 1, only E) eq2: an operand (non splitable) eq3:
	 *         second part of equation (this should be in alreadySolved) Or a
	 *         list containing the same equation as given (thus one element) ->
	 *         happens when equation did not contain any splitable parts
	 *
	 *
	 *         f.e. equation = E+E*E return = {E, +, E*E} (all elements are
	 *         equations)
	 */
	 ...
	 *            The equation to be split
	 * @return A list containing three equations eq1: first part of equation
	 *         eq2: an operand (splitable) eq3: second part of equation (after
	 *         operand) Or a list containing the same equation as given (thus
	 *         one element) -> happens when equation did not contain any
	 *         splitable parts
	 *
	 *
	 *         f.e. equation = E+E*E return = {E, +, E*E} (all elements are
	...
	public List<Equation> splitSplitableTwoParts(Equation eq) {
		...
		if (seconPart.isEmpty()) {
			// if no parts were added to the second part this means that the
			// first part is not added to split yet
			split.add(new Equation(firstPart));
		} else {
			split.add(new Equation(seconPart));
		}
