...
public class StringEvaluate{
	...
	public StringEvaluate(Tree tree, ArrayList<Double> mainExample){
		...
		this.MAIN_EXAMPLE = mainExample.subList(0, mainExample.size()-1);
		...
		for(int i = 0; i < MAIN_EXAMPLE.size(); i++) {
			if(!nameByValue.containsKey(mainExample.get(i))) {
				ArrayList<String> newList = new ArrayList<String>();
				newList.add("K"+(i+1));
				nameByValue.put(MAIN_EXAMPLE.get(i), newList);
			} else {
		...
		for(List<Double> example: getAllExamples()) {
			...
			for(String str : listOfParts) {
				...
				if(str.length() < 3 && !Grammar.isOperand(str)) {
					...
					//List<String> splitStr = splitStringSplittable(str);
